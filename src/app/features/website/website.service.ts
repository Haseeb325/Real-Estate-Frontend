...
export class WebSiteService extends BaseCrudService<Property> {
  ...
  fetchProperties(params: any, append = false, forceRefresh = false) {
    return this.fetch(
      this.apiService.getWithParams(URLConfig.getAllProperties, params),
      forceRefresh,
      append,
    );
  }
  ...
  getPropertyAvailability(id: any) {
    return this.apiService.get(URLConfig.getPropertyAvailability(id));
  }

  createAppointment(payload: any) {
    return this.apiService.post(URLConfig.appointments, payload);
  }
}
