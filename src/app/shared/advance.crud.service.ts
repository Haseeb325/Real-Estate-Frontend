...
export abstract class AdvanceCrudService<TModel, TDto = any> {
  ...
  async create(body: postRequestOptions<TModel>) {
    const { url, payload, isFormData, autoRefresh, autoRefreshUrl, updateState, useGlobalLoading } =
      body;
      ...
      if (updateState) {
        if (Array.isArray(mappedData)) {
          ...
          this._items.update((items) =>
            items.some((i) => (i as any)?.id === (mappedData as any)?.id)
              ? items
              : [...items, mappedData],
          );
        }
  ...
  async update(options: postRequestOptions<TModel>) {
    const { url, payload, isFormData, autoRefresh, useGlobalLoading } = options;
